package v9;

import java.util.Arrays;
import java.util.Random;

/* loaded from: classes3.dex */
public interface P {

    public static class bar implements P {

        /* renamed from: a, reason: collision with root package name */
        public final Random f203644a;

        /* renamed from: b, reason: collision with root package name */
        public final int[] f203645b;

        /* renamed from: c, reason: collision with root package name */
        public final int[] f203646c;

        public bar() {
            this(new Random());
        }

        @Override // v9.P
        public final bar a(int i11, int i12) {
            int i13 = i12 - i11;
            int[] iArr = this.f203645b;
            int[] iArr2 = new int[iArr.length - i13];
            int i14 = 0;
            for (int i15 = 0; i15 < iArr.length; i15++) {
                int i16 = iArr[i15];
                if (i16 < i11 || i16 >= i12) {
                    int i17 = i15 - i14;
                    if (i16 >= i11) {
                        i16 -= i13;
                    }
                    iArr2[i17] = i16;
                } else {
                    i14++;
                }
            }
            return new bar(iArr2, new Random(this.f203644a.nextLong()));
        }

        @Override // v9.P
        public final bar cloneAndClear() {
            return new bar(new Random(this.f203644a.nextLong()));
        }

        @Override // v9.P
        public final bar cloneAndInsert(int i11, int i12) {
            int[] iArr;
            Random random;
            int[] iArr2 = new int[i12];
            int[] iArr3 = new int[i12];
            int i13 = 0;
            while (true) {
                iArr = this.f203645b;
                random = this.f203644a;
                if (i13 >= i12) {
                    break;
                }
                iArr2[i13] = random.nextInt(iArr.length + 1);
                int i14 = i13 + 1;
                int iNextInt = random.nextInt(i14);
                iArr3[i13] = iArr3[iNextInt];
                iArr3[iNextInt] = i13 + i11;
                i13 = i14;
            }
            Arrays.sort(iArr2);
            int[] iArr4 = new int[iArr.length + i12];
            int i15 = 0;
            int i16 = 0;
            for (int i17 = 0; i17 < iArr.length + i12; i17++) {
                if (i15 >= i12 || i16 != iArr2[i15]) {
                    int i18 = i16 + 1;
                    int i19 = iArr[i16];
                    iArr4[i17] = i19;
                    if (i19 >= i11) {
                        iArr4[i17] = i19 + i12;
                    }
                    i16 = i18;
                } else {
                    iArr4[i17] = iArr3[i15];
                    i15++;
                }
            }
            return new bar(iArr4, new Random(random.nextLong()));
        }

        @Override // v9.P
        public final int getFirstIndex() {
            int[] iArr = this.f203645b;
            if (iArr.length > 0) {
                return iArr[0];
            }
            return -1;
        }

        @Override // v9.P
        public final int getLastIndex() {
            int[] iArr = this.f203645b;
            if (iArr.length > 0) {
                return iArr[iArr.length - 1];
            }
            return -1;
        }

        @Override // v9.P
        public final int getLength() {
            return this.f203645b.length;
        }

        @Override // v9.P
        public final int getNextIndex(int i11) {
            int i12 = this.f203646c[i11] + 1;
            int[] iArr = this.f203645b;
            if (i12 < iArr.length) {
                return iArr[i12];
            }
            return -1;
        }

        @Override // v9.P
        public final int getPreviousIndex(int i11) {
            int i12 = this.f203646c[i11] - 1;
            if (i12 >= 0) {
                return this.f203645b[i12];
            }
            return -1;
        }

        public bar(int[] iArr, Random random) {
            this.f203645b = iArr;
            this.f203644a = random;
            this.f203646c = new int[iArr.length];
            for (int i11 = 0; i11 < iArr.length; i11++) {
                this.f203646c[iArr[i11]] = i11;
            }
        }

        public bar(Random random) {
            this(new int[0], random);
        }
    }

    bar a(int i11, int i12);

    bar cloneAndClear();

    bar cloneAndInsert(int i11, int i12);

    int getFirstIndex();

    int getLastIndex();

    int getLength();

    int getNextIndex(int i11);

    int getPreviousIndex(int i11);
}
